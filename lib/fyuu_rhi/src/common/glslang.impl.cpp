...
namespace {
	...
	TBuiltInResource s_glslang_resources;
	
	TBuiltInResource InitGLSLangResources() {
	...
	EShLanguage ShaderStageToEShLanguage(ShaderStage shader_stage) {
		switch (shader_stage) {
		...
		case fyuu_rhi::ShaderStage::Fragment:
	...
	std::vector<std::uint32_t> CompileShader(
		...
		*/
		...
		if (language == glslang::EShSourceHlsl) {
			input_version = 2018; // HLSL 2018
		}

		shader.setEnvInput(
			language,
			glslang_shader_stage,
		...
		shader.setEntryPoint(option.entry_point.data());
...
namespace fyuu_rhi {
	...
	std::vector<std::uint32_t> CompileGLSLToSPIRV(
		...
		return CompileShader(src, shader_stage, option, glslang::EShSource::EShSourceGlsl);
	}
	...
	std::vector<std::uint32_t> CompileHLSLToSPIRV(
		...
		return CompileShader(src, shader_stage, option, glslang::EShSource::EShSourceHlsl);
	}
