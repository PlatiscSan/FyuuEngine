...
export namespace fyuu_engine::concurrency {
	...
	public:
		...
		class BucketAccessingPointer {
		private:
			mapped_type const* m_mapped;
			std::shared_lock<std::shared_mutex> m_lock;

		public:
			BucketAccessingPointer(mapped_type const* mapped, std::shared_lock<std::shared_mutex>&& lock)
				: m_mapped(mapped),
				m_lock(std::move(lock)) {

			}

			BucketAccessingPointer(std::nullptr_t) noexcept
				:m_mapped(nullptr) {

			}

			~BucketAccessingPointer() noexcept = default;
	...
	private:
		...
		template <class K, class... Args>
		std::pair<mapped_type*, bool> BucketUnsafeEmplaceImpl(K&& key, Args&&... args) {
			...
			return {
				std::piecewise_construct_t(),
				std::forward_as_tuple(&emplaced.second),
				std::forward_as_tuple(true)
			};

		}

		bool EraseImpl(Key const& key) {
			size_type idx = BucketIndex(key);
			Bucket& bucket = m_buckets[idx];
	...
	public:
		...
		BucketAccessingPointer find(Key const& key) const {
		...
		bool erase(Key const& key) {
			...
			auto lock = m_buckets_mutex.Lock();
		...
		template <class... Keys>
		std::bitset<sizeof...(Keys)> EraseKeys(Keys&&... keys) {

			std::bitset<sizeof...(Keys)> results(false);
			...
			size_t index = 0;
			((results[index++] = EraseImpl(std::forward<Keys>(keys))), ...);

			return results;

		}

		std::vector<bool> EraseKeys(std::span<Key const> keys) {
		...
		bool UnsafeErase(Key const& key) {
		...
		template <class K, class... Args>
		std::pair<mapped_type*, bool> BucketUnsafeEmplace(K&& key, Args&&... args) {
			return BucketUnsafeEmplaceImpl(std::forward<K>(key), std::forward<Args>(args)...);
		}

		bool BucketUnsafeErase(Key const& key) {
...
export namespace std {
	template <class Key>
	struct hash<fyuu_engine::concurrency::ImmutableKey<Key>> {
		std::size_t operator()(fyuu_engine::concurrency::ImmutableKey<Key> const& key) const {
