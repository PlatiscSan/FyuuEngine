...
namespace fyuu_engine::physics {
	...
	private:
		...
		*/
		...
		std::uint32_t CreateRigidBodyImpl(Base::Transform const& global_transform, Base::RigidBodyComponentResource const& rigid_body_actor_resource) {
			
			JPH::BodyInterface& body_interface = s_jolt_physics_system->GetBodyInterface();

			struct JPHShapeData {
				JPH::Shape* shape{ nullptr };
				Base::Transform   local_transform;
				Base::Vector3     global_position;
				Base::Vector3     global_scale;
				Base::Quaternion  global_rotation;
			};
			...
			for (auto const& shape : shapes) {
				...
				typename Base::Matrix shape_global_transform = global_transform.
