...
namespace fyuu_engine::util {
	...
	public:
		...
		class BaseSubscription : public std::enable_shared_from_this<BaseSubscription> {
		protected:
		...
		public:
			class Collector {
			private:
				std::weak_ptr<EventBus::BaseSubscription> m_owner;
			public:
				Collector(std::weak_ptr<EventBus::BaseSubscription> const& owner)
					: m_owner(owner) {
			...
			using SubscriptionHandle = util::UniqueCollectiveResource<std::size_t, Collector>;

			BaseSubscription() = default;

			virtual ~BaseSubscription() noexcept = default;
	...
	private:
		...
		class DerivedSubscription : public EventBus::BaseSubscription {
		private:
			using Base = EventBus::BaseSubscription;
		...
		public:
			...
			SubscriptionHandle Subscribe(Action const& action) override {
				...
				return UniqueCollectiveResource<std::size_t, Collector>(id, this->weak_from_this());
		...
		std::unordered_map<std::string, std::shared_ptr<BaseSubscription>> m_events;

		mutable std::shared_mutex m_events_mutex;

	public:
		using SubscriptionHandle = util::EventBus::BaseSubscription::SubscriptionHandle;

		template <class Event>
		void Publish(Event&& event) const {

			std::shared_ptr<BaseSubscription> subscriptions;
